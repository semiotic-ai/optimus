...
const CONTRACT_NAME: &'static str = "positionmanager";
...
pub mod events {
    ...
    impl Events {
        pub fn match_and_decode(
            ...
            if let Some(event) = DecreaseLiquidity::match_and_decode(log) {
                return Some(Events::DecreaseLiquidity(event));
            }
            if let Some(event) = IncreaseLiquidity::match_and_decode(log) {
                return Some(Events::IncreaseLiquidity(event));
            }
