...
const CONTRACT_NAME: &'static str = "pool";
...
pub mod events {
    ...
    impl Events {
        pub fn match_and_decode(
            ...
            if let Some(event) = Burn::match_and_decode(log) {
                return Some(Events::Burn(event));
            }
            if let Some(event) = Collect::match_and_decode(log) {
                return Some(Events::Collect(event));
            }
            if let Some(event) = CollectProtocol::match_and_decode(log) {
                return Some(Events::CollectProtocol(event));
            }
            if let Some(event) = Flash::match_and_decode(log) {
                return Some(Events::Flash(event));
            }
            if let Some(event)
                = IncreaseObservationCardinalityNext::match_and_decode(log) {
                return Some(Events::IncreaseObservationCardinalityNext(event));
            }
            if let Some(event) = Initialize::match_and_decode(log) {
                return Some(Events::Initialize(event));
            }
            if let Some(event) = Mint::match_and_decode(log) {
                return Some(Events::Mint(event));
            }
            if let Some(event) = SetFeeProtocol::match_and_decode(log) {
                return Some(Events::SetFeeProtocol(event));
            }
            if let Some(event) = Swap::match_and_decode(log) {
                return Some(Events::Swap(event));
            }
            return None;
        }
    }
