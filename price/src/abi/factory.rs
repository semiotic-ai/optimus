...
const CONTRACT_NAME: &'static str = "factory";
...
pub mod events {
    ...
    impl Events {
        pub fn match_and_decode(
            ...
            if let Some(event) = FeeAmountEnabled::match_and_decode(log) {
                return Some(Events::FeeAmountEnabled(event));
            }
            if let Some(event) = OwnerChanged::match_and_decode(log) {
                return Some(Events::OwnerChanged(event));
            }
            if let Some(event) = PoolCreated::match_and_decode(log) {
                return Some(Events::PoolCreated(event));
            }
            return None;
        }
    }
